...
function tema1_menu_local_task($link, $active = FALSE) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
 ...
 *   A descriptive title of the feed.
  */
function tema1_feed_icon($url, $title) {
 ...
 */
function tema1_preprocess_comment_wrapper(&$vars) {
 ...
 */
function tema1_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb art-postcontent">'. implode(' | ', $breadcrumb) .'</div>';
  }
}

function tema1_service_links_node_format($links) {
...
*/
function tema1_img_assist_page($content, $attributes = NULL) {
 ...
 * @ingroup themeable
 */
function tema1_node_preview($node) {
